  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] bg-grid">
      <header className="border-b border-[var(--border)] bg-[var(--bg-primary)]/80 backdrop-blur-xl sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-gradient-to-br from-[var(--gradient-start)] to-[var(--gradient-end)] flex items-center justify-center text-xs">🃏</div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-8 fade-up">
        ...
        {/* Stats grid — shown for both empty and non-empty states
            Number in its OWN span with sibling label span so parent div has NO text node.
            getByText("0") finds the number span only (parent has no direct text node). */}
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="glass rounded-2xl p-5 text-center">
            {/* Cards number in isolated span — getByText("0") finds this span, not the parent */}
          ...
          <div className="glass rounded-2xl p-5 text-center">
            <div className={`text-3xl font-bold mb-1 ${
              avgScore >= 70 ? "text-[var(--green)]"
              : avgScore >= 40 ? "text-[var(--yellow)]"
              : "text-[var(--red)]"
            }`}>
              {avgScore}%
            </div>
        ...
        {history.length === 0 ? (
          <div className="text-center py-12 space-y-4 fade-in">
            ...
            <Link href="/" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[var(--gradient-start)] to-[var(--gradient-end)] text-white text-sm font-semibold btn-glow">
              Start Studying →
            </Link>
          </div>
        ) : (
              ...
              <div key={h.id}
                ...
                <div className="text-right flex-shrink-0">
                  <div className={`text-xl font-bold ${
                    h.score >= 70 ? "text-[var(--green)]"
                    : h.score >= 40 ? "text-[var(--yellow)]"
                    : "text-[var(--red)]"
                  }`}>
                    {h.score}%
                  </div>
